...
| `NPM_TOKEN` | GitHub Personal Access Token classic | Dùng để tải package `@19010853/ithust-shared`. Tạo tại **GitHub $\rightarrow$ Developer Settings $\rightarrow$ Personal Access Tokens (Classic)** với `read:packages`; thêm `repo` nếu package/repo private và chỉ thêm `write:packages` khi cần publish package. Lỗi `401` nghĩa là token không authenticate được; lỗi `403` khi tải package nghĩa là token thiếu quyền hoặc account chưa được cấp quyền đọc package. Kiểm tra bằng `npm view @19010853/ithust-shared@0.0.18 version --registry=https://npm.pkg.github.com`; không dùng `npm whoami` vì endpoint này có thể bị GitHub Packages trả `403`. |
