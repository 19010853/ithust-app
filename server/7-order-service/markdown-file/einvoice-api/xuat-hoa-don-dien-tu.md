...
<ErrorCodes
  ...
  { code: 422, name: "INVOICE_DATE_BEFORE_TAX_APPROVAL", description: "issued_date trước ngày CQT duyệt. Xem tax_authority_approved_date trong GET /v1/provider-accounts." },
...
<Callout type="info" title="Số lượng thập phân">
`quantity`
 chấp nhận giá trị thập phân (tối đa 2 chữ số sau dấu phẩy).
Phù hợp cho dịch vụ tính giờ (
`Giờ`
), cân/đo lường (
`Kg`
, 
...
<Callout type="warning" title="issued_date">
`issued_date`
 phải từ 
`tax_authority_approved_date`
 trở đi. Sớm hơn sẽ nhận 422 
`INVOICE_DATE_BEFORE_TAX_APPROVAL`
.
Ngày này lấy ở 
