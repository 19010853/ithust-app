...
import { IAuthBuyerMessageDetails, IAuthDocument, firstLetterToUppercase, lowerCase, winstonLogger } from '@19010853/ithust-shared';
...
export async function getUserByUsernameOrEmail(username: string, email: string): Promise<IAuthDocument | undefined> {
    try {
        const user: Model = await AuthModel.findOne({
            where: {
                [Op.or]: [{ username: firstLetterToUppercase(username) }, { email: lowerCase(email) }]
            },
        }) as Model;
        return user?.dataValues;
    } catch (error) {
