...
import { BadRequestError, IBuyerDocument, IEducation, IExperience, ISellerDocument } from '@19010853/ithust-shared';
...
const randomExperiences = (count: number): IExperience[] => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const randomEndYear = ['Present', '2026', '2027', '2028', '2029', '2030'];
...
const randomEducation = (count: number): IEducation[] => {
    ...
    for (let i = 0; i < count; i++) {
        const randomYear = [2020, 2021, 2022, 2023, 2024, 2025, 2026];
        const education = {
            country: faker.location.country(),
