...
const VerifyOTP: FC = (): ReactElement => {
  ...
  const onVerifyOTP = async (): Promise<void> => {
    try {
      ...
      const buyerResponse: IResponse = await dispatch(buyerApi.endpoints.getCurrentBuyerByUsername.initiate()).unwrap();
      dispatch(addBuyer(buyerResponse.buyer));
      const sellerResponse: IResponse = await dispatch(
        sellerApi.endpoints.getSellerByUsername.initiate(`${result.user?.username}`)
      ).unwrap();
      dispatch(addSeller(sellerResponse.seller));
    ...
    return (
      <>
        ...
        <div className="container mx-auto flex flex-col items-center justify-center px-6 py-8 mt-32 lg:py-0">
          <div className="md:w-[30%] font-bold border-b pb-2 sm:w-[60%]">Xác thực</div>
          <div className="md:w-[30%] flex flex-col justify-center sm:w-[60%]">
            ...
            <p className="text-center mb-3">
              Vui lòng kiểm tra hộp thư, chúng tôi đã gửi mã hết hạn sau <span className="font-bold">10 phút</span>.
            </p>
            ...
            <div className="flex w-full items-center justify-center">
              <Button
                ...
                label={`${isLoading ? 'ĐANG XÁC MINH...' : 'XÁC MINH'}`}
                onClick={onVerifyOTP}
              />
            </div>
          </div>
