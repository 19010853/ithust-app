...
const disputeStatusLabel: Record<IDisputeDocument['status'], string> = {
  ...
  REFUND_BUYER: 'Đã hoàn tiền buyer',
  RELEASE_SELLER: 'Đã giải ngân seller',
  CLOSED: 'Đã đóng'
};
...
const AdminDisputes: FC = (): ReactElement => {
  ...
  const [sendMessage, { isLoading: isSendingMessage }] = useCreateDisputeMessageMutation();
  const [decide, { isLoading: isDeciding }] = useDecideDisputeMutation();
  const isResolved = Boolean(selected && ['REFUND_BUYER', 'RELEASE_SELLER', 'CLOSED'].includes(selected.status));
    ...
    <div className="container mx-auto mt-8 px-4">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <div className="space-y-3">
          {disputes.map((item) => {
            const isCardCollapsed = Boolean(item._id && collapsedIds[item._id]);
            return (
              <div
                key={item._id}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="font-bold">
                    #{item.orderId} · {disputeStatusLabel[item.status]}
                  </div>
                  <div
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      if (!item._id) return;
                      setCollapsedIds((prev) => ({ ...prev, [item._id as string]: !prev[item._id as string] }));
                    }}
                  >
                    {isCardCollapsed ? <FaChevronDown size={15} /> : <FaChevronUp size={15} />}
                  </div>
                </div>
                {!isCardCollapsed && (
                  <>
        ...
        {selected && (
          <div className="rounded border p-4">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="font-bold">Trao đổi #{selected.orderId}</h2>
              <div className="cursor-pointer" onClick={() => setShowDetailPanel((item) => !item)}>
                {!showDetailPanel ? <FaChevronDown size={15} /> : <FaChevronUp size={15} />}
              </div>
            </div>
            {showDetailPanel && (
              <>
                ...
                <Button
                  className="mt-2 rounded bg-sky-500 px-3 py-2 text-white disabled:opacity-50"
                  label={isSendingMessage ? 'Đang gửi...' : 'Gửi tin nhắn'}
                ...
                {isResolved ? (
                  <div className="rounded bg-gray-50 p-3 text-sm">
                    <p className="font-bold">Đã xử lý: {disputeStatusLabel[selected.status]}</p>
                    {selected.decisionReason && <p className="mt-1 text-gray-600">Lý do: {selected.decisionReason}</p>}
                  </div>
                ) : (
                  <>
                    ...
                    <Button
                      className="mt-2 rounded bg-emerald-600 px-3 py-2 text-white disabled:opacity-50"
                      label={isDeciding ? 'Đang xử lý...' : 'Xác nhận quyết định'}
                      disabled={isDeciding}
